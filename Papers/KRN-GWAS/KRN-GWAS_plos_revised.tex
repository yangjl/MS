...
During the years 2008-2011, subsets of the above populations were planted using an incomplete block design in upto three replicated fields in Ames, IA (summer season) and one field in Molokai, HI (winter season). The block size was 50 rows. There were 5-12 plants of the same entry grown within each row. On average, $~$2,000 entries were tested in a trial. All plants in each row were harvested and KRN counts were collected from mature ears. Phenotypic values were estimated for each entry using a mixed linear model implemented in R ~\cite{DevelopmentCoreTeam2011}, with fixed effects for entries and random effects for fields, locations and years. Phenotypic density distributions in this study were estimated and plotted using R with default smoothing parameters. 
