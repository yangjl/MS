...
During the years 2008-2011, the above populations were sequentially planted in Ames, IA (summer season) and in Molokai, HI (winter season). In each location of a year, subsets of entries (selected according to the similarity of the entries, i.e. several NAM subpopulations or a diallel population) were planted. Our experiment was an incomplete block design, where each location of a year was considered as a block. On average, $\sim$1,000 entries were planted in a block and 5-12 plants of the same entry were grown within each row. The blocks forms one complete replication of all entries, therefore the design is resolvable. All plants in each row were harvested and KRN counts were collected from mature ears. 
Phenotypic values were estimated for each entry using a mixed linear model implemented in R ~\cite{DevelopmentCoreTeam2011}, with fixed effects for entries and random effects for locations, years and rows. The above terms explained 19\%, 7\% and 0.3\% of the phenotypic variance, respectively. We also included entries by locations and entries by years as random effects, but dropped these terms because they did not significantly improve the previous model.
In addition to our trial, we obtained eight trials of KRN data from Brown et al. ~\cite{Brown2011}. A heterogeneity checking for error variances (Bartlett’s test P value $<$ 0.01) indicated that at least two trial-specific error variances were different. We fitted the BLUE values of the nine trials in a mixed linear model with the trial as random effect.
Phenotypic density distributions in this study were estimated and plotted using R with default smoothing parameters. 
...
We independently estimated allele frequencies obtained from the available parental lines and from the C0 seed. Allele frequencies of the two sets were significantly correlated (Pearson correlation \emph{P} value $<$ 0.05).
Therefore, the allele frequencies for C0 were estimated by combining the data from parental lines and C0 seeds. Population allele frequencies for C30 LE and C30 SE were estimated based on the surveyed samples. 
...
A simulation method (Hinze, 2003) was employed to mimic the selection procedure. If observed allele frequency changes significantly lager than simulations, we can rule out the possibilities of genetic drift or stochastic sampling error for the locus.
After simulation, one validated KAV did not pass the cutoff (FDR $<$ 0.05) and was removed. Hence, even after accounting for drift and stochastic sampling errors, 6/51 (12\%) KAVs were deemed to have been under divergent selection. 
We applied the simulation to the negative control variants and they were indeed negative (Figure 4, \nameref{Table_S9}, \nameref{Table_S12} and \nameref{Table_S13}).
