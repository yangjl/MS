...
During the years 2008-2011, the above populations were planted in up to three locations in Ames, IA (summer season) and one location in Molokai, HI (winter season). In each location, subsets of entries were planted in field blocks of 50 rows and 5-12 plants of the same entry were grown within each row. On average, $~$1,000 entries were planted in a location. All plants in each row were harvested and KRN counts were collected from mature ears. Phenotypic values were estimated for each entry using a mixed linear model implemented in R ~\cite{DevelopmentCoreTeam2011}, with fixed effects for entries and random effects for locations, years, replications, blocks and rows. The above terms explained 19\%, 7\%, 3\%, 2\% and 0.3\% of the phenotypic variance, respectively. We also included entries by locations and entries by years as random effects, but dropped these terms because they did not dramatically improve the previous model. Phenotypic density distributions in this study were estimated and plotted using R with default smoothing parameters. 
...
A simulation procedure, not developed for the initial association loci discovering, which mimicked the selection program was conducted to test whether observed changes in allele frequency were larger than expected by genetic drift or stochastic sampling error.
