...
During the years 2008-2011, \DIFdelbegin \DIFdel{subsets of }\DIFdelend the above populations were \DIFdelbegin \DIFdel{planted using an incomplete block design in upto three replicated fields in }\DIFdelend \DIFaddbegin \DIFadd{sequentially planted in }\DIFaddend Ames, IA (summer season) and \DIFdelbegin \DIFdel{one field }\DIFdelend in Molokai, HI (winter season). \DIFdelbegin \DIFdel{The block size was 50 rows. There were }\DIFdelend \DIFaddbegin \DIFadd{In each location of a year, subsets of entries (selected according to the similarity of the entries, i.e. several NAM subpopulations or a diallel population) were planted. Our experiment was an incomplete block design, where each location of a year was considered as a block. On average, ~1,000 entries were planted in a block and }\DIFaddend 5-12 plants of the same entry \DIFaddbegin \DIFadd{were }\DIFaddend grown within each row. \DIFdelbegin \DIFdel{On average, $~$2,000 entrieswere tested in a trial}\DIFdelend \DIFaddbegin \DIFadd{The blocks forms one complete replication of all entries, therefore the design is not resolvable}\DIFaddend . All plants in each row were harvested and KRN counts were collected from mature ears. 
...
}. A heterogeneity checking (Bartlett’s test P value $<$ 0.01) indicated that at least two trial-specific error variances were different. We fitted the BLUE values of the nine trials in a mixed linear model with the trial as random effect.
