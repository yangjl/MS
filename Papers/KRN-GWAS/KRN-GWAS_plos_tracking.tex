...
During the years 2008-2011, \DIFdelbegin \DIFdel{subsets of }\DIFdelend the above populations were planted \DIFdelbegin \DIFdel{using an incomplete block design in replicated field trials in }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend up to three \DIFdelbegin \DIFdel{fields }\DIFdelend \DIFaddbegin \DIFadd{locations }\DIFaddend in Ames, IA (summer season) and one \DIFdelbegin \DIFdel{field }\DIFdelend \DIFaddbegin \DIFadd{location }\DIFaddend in Molokai, HI (winter season). \DIFdelbegin \DIFdel{There were }\DIFdelend \DIFaddbegin \DIFadd{In each location, subsets of entries were planted in field blocks of 50 rows and }\DIFaddend 5-12 plants of the same \DIFdelbegin \DIFdel{line }\DIFdelend \DIFaddbegin \DIFadd{entry were }\DIFaddend grown within each row. \DIFaddbegin \DIFadd{On average, $~$1,000 entries were planted in a location. }\DIFaddend All plants in each row were harvested and KRN counts were collected from mature ears. Phenotypic values were estimated for each \DIFdelbegin \DIFdel{line }\DIFdelend \DIFaddbegin \DIFadd{entry }\DIFaddend using a mixed linear model implemented in R ~\cite{DevelopmentCoreTeam2011}, with fixed effects for entries and random effects for \DIFdelbegin \DIFdel{trials, }\DIFdelend locations, years, \DIFdelbegin \DIFdel{plots and blocks . }\DIFdelend \DIFaddbegin \DIFadd{replications, blocks and rows. The above terms explained 19\%, 7\%, 3\%, 2\% and 0.3\% of the phenotypic variance, respectively. We also included entries by locations and entries by years as random effects, but dropped these terms because they did not dramatically improve the previous model. }\DIFaddend Phenotypic density distributions in this study were estimated and plotted using R with default smoothing parameters. 
...
A simulation procedure\DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{, not developed for the initial association loci discovering, which }\DIFaddend mimicked the selection program was conducted to test whether observed changes in allele frequency were larger than expected by genetic drift or stochastic sampling error.
