...
%DIF DEL KRN-GWAS_plos.tex           Mon Jul  6 11:05:36 2015
%DIF ADD KRN-GWAS_plos_revised.tex   Mon Jul  6 11:48:19 2015
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
%DIF <  % % SUPPORTING INFORMATION % % %
%DIF <  % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\section*{\DIFdel{Supporting Information}}
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Include only the SI item label in the subsection heading. Use the \nameref{label} command to cite SI items in the text.
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S1 Fig}}
%DIFAUXCMD
%DIFDELCMD < \label{Fig_S1}
%DIFDELCMD < {\bf %%%
\DIFdel{High parent heterosis (HPH) and mid-parent heterosis (MPH) of the KRN trait in three hybrid populations.}%DIFDELCMD < } %%%
\DIFdel{Vertical dashed lines indicate no heterosis.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S2 Fig}}
%DIFAUXCMD
%DIFDELCMD < \label{Fig_S2}
%DIFDELCMD < {\bf %%%
\DIFdel{Phenotypic distribution of the B73, Mo17 and their reciprocal F1 hybrids.}%DIFDELCMD < } %%%
\DIFdel{Replicated observations of B73 x Mo17 (KNR = 15.5 $\pm$ 1.4) and Mo17 x B73 (KRN = 15.6 $\pm$ 1.4) exhibited similar mean and variance of phenotypic distributions, where the mean values of KRN fall in between of the mean KRN values from B73 (KRN = 17.1) and Mo17 (KRN = 10.8). Density curves were smoothed with default bandwidth parameters.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S3 Fig}}
%DIFAUXCMD
%DIFDELCMD < \label{Fig_S3}
%DIFDELCMD < {\bf %%%
\DIFdel{Venn diagram comparing KAV bins identified by the three different approaches.}%DIFDELCMD < } %%%
\DIFdel{After applying a variant thinning procedure, 192 100-kb bins (257 variants), 296 bins (300 variants) and 343 bins (442 variants) were identified using arbitrary thresholds for the single-variant, stepwise regression and Bayesian-based multi-variant approaches, respectively. Note in the parentheses listed the unique KAV numbers in the bins.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S4 Fig}}
%DIFAUXCMD
%DIFDELCMD < \label{Fig_S4}
%DIFDELCMD < {\bf %%%
\DIFdel{Single variant effect and heritability of the 231 KAVs.}%DIFDELCMD < } %%%
\DIFdel{In panel (a) and (b), histogram distributions of single variant effect and heritability were computed for the 231 KAVs individually. Note that because of linkage disequilibrium (LD), effect and heritability might be repeatedly calculated.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S5 Fig}}
%DIFAUXCMD
%DIFDELCMD < \label{Fig_S5}
%DIFDELCMD < {\bf %%%
\DIFdel{Characteristics of the ~13M variants and 231 KAVs. }%DIFDELCMD < } %%%
\DIFdel{In upper panels (a, b and c), pie charts show the physical distance to the nearest gene, origin and type of variation of the $\sim$13M variants used for the GWAS, respectively; in lower panels (d, e and f), show the same characteristics of 231 KAVs. The star indicates significant difference after Chi-square tests.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S6 Fig}}
%DIFAUXCMD
%DIFDELCMD < \label{Fig_S6}
%DIFDELCMD < {\bf %%%
\DIFdel{Genetic and physical positions of the KAVs across 10 maize chromosomes.}%DIFDELCMD < } %%%
\DIFdel{Red dots represent the genetic validated KAVs and blue dots are not validated KAVs.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S1 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S1}
%DIFDELCMD < {\bf %%%
\DIFdel{KRN trait data of the 6,230 lines.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S2 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S2}
%DIFDELCMD < {\bf %%%
\DIFdel{Separate QTL results of IBM and NAM RILs.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S3 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S3}
%DIFDELCMD < {\bf %%%
\DIFdel{Four QTL regions exhibit opposite effects in different NAM RIL subpopulations.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S4 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S4}
%DIFDELCMD < {\bf %%%
\DIFdel{Joint QTL results of 25 NAM RIL subpopulations.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S5 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S5}
%DIFDELCMD < {\bf %%%
\DIFdel{KAVs identified by three GWAS approaches.}%DIFDELCMD < } %%%
\DIFdel{Column “value” are significant measurements of different approaches, where –log10(P) values for single-variant approach, F-test statistic values for stepwise regression approach, and posterior model frequencies (MF) for Bayesian-based multi-variant approach. Binsize  = 100-kb.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S6 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S6}
%DIFDELCMD < {\bf %%%
\DIFdel{Effects of KAVs in the four QTL regions.}%DIFDELCMD < } %%%
\DIFdel{In QTL regions of order 1 and 2, both positive and negative effects were observed for identified KAVs. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S7 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S7}
%DIFDELCMD < {\bf %%%
\DIFdel{KThe 231 KAVs selected for cross-validation.}%DIFDELCMD < } %%%
\DIFdel{Note some of the KAVs were identified by multiple approaches. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S8 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S8}
%DIFDELCMD < {\bf %%%
\DIFdel{Favorable allele frequencies and statistical test results of historical lines. }%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S9 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S9}
%DIFDELCMD < {\bf %%%
\DIFdel{Cross-validation samples.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S10 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S10}
%DIFDELCMD < {\bf %%%
\DIFdel{Genotypic data for cross-validation population of elite inbred lines.}%DIFDELCMD < } %%%
\DIFdel{Genotypes were coded using 3 for reference-like, 2 for heterozygotes, 1 for non-reference-like variant types and 0 for missing.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S11 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S11}
%DIFDELCMD < {\bf %%%
\DIFdel{Genotypic data for cross-validation population of USDA PI accessions.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S12 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S12}
%DIFDELCMD < {\bf %%%
\DIFdel{Genotypic data for cross-validation population of BSLE.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S13 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S13}
%DIFDELCMD < {\bf %%%
\DIFdel{Summary of cross-validation results.}%DIFDELCMD < } 
%DIFDELCMD < %%%
\DIFdel{Columns of “Elite1}%DIFDELCMD < \_Qvalue%%%
\DIFdel{” and “Elite2}%DIFDELCMD < \_Qvalue%%%
\DIFdel{” are cross-validation results using elite inbred population with and without population structure control. Columns of “USDA}%DIFDELCMD < \_Qvalue%%%
\DIFdel{” and “BSLE}%DIFDELCMD < \_Qvalue%%%
\DIFdel{” are cross-validation results using USDA germplasm accessions and BSLE population. Columns “Elite1}%DIFDELCMD < \_DOE%%%
\DIFdel{”, “Elite2}%DIFDELCMD < \_DOE%%%
\DIFdel{”, “USDA}%DIFDELCMD < \_DOE%%%
\DIFdel{” and “BSLE}%DIFDELCMD < \_DOE%%%
\DIFdel{” are products of direction of effects (DOE) of KAVs in the GWAS population and the cross-validation populations, where positive numbers indicated consistency and negative number indicated inconsistency of DOE. “NA” in the table indicated the data are not available. Significant KAVs were indicated by stars ($\ast$) in the column of “SNPID”.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection*{\DIFdel{S14 Table}}
%DIFAUXCMD
%DIFDELCMD < \label{Table_S14}
%DIFDELCMD < {\bf %%%
\DIFdel{List of candidate genes within KAV-associated chromosomal bins that aligned to evidence supported genes.}%DIFDELCMD < }
%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{This research was supported by grants from Monsanto and the National Science Foundation (IOS-1027527) to PSS.
}\DIFdelend 
...
\DIFdelbegin %DIFDELCMD < \bibitem{Larsson2013}
%DIFDELCMD < %%%
\DIFdel{Larsson SJ, Lipka AE, Buckler ES.
}%DIFDELCMD < \newblock {%%%
\DIFdel{Lessons from Dwarf8 on the Strengths and Weaknesses of Structured
  Association Mapping}%DIFDELCMD < }%%%
\DIFdel{.
}%DIFDELCMD < \newblock %%%
\DIFdel{PLoS Genetics. 2013;9(2).
}\DIFdelend %DIF > \bibitem{Larsson2013}
%DIF > Larsson SJ, Lipka AE, Buckler ES.
%DIF > \newblock {Lessons from Dwarf8 on the Strengths and Weaknesses of Structured
%DIF >   Association Mapping}.
...
\section*{\DIFadd{Supporting Information}}

%DIF >  Include only the SI item label in the subsection heading. Use the \nameref{label} command to cite SI items in the text.

\subsection*{\DIFadd{S1 Fig}}
\label{Fig_S1}
{\bf \DIFadd{High parent heterosis (HPH) and mid-parent heterosis (MPH) of the KRN trait in three hybrid populations.}} \DIFadd{Vertical dashed lines indicate no heterosis.
}

\subsection*{\DIFadd{S2 Fig}}
\label{Fig_S2}
{\bf \DIFadd{Phenotypic distribution of the B73, Mo17 and their reciprocal F1 hybrids.}} \DIFadd{Replicated observations of B73 x Mo17 (KNR = 15.5 $\pm$ 1.4) and Mo17 x B73 (KRN = 15.6 $\pm$ 1.4) exhibited similar mean and variance of phenotypic distributions, where the mean values of KRN fall in between of the mean KRN values from B73 (KRN = 17.1) and Mo17 (KRN = 10.8). Density curves were smoothed with default bandwidth parameters.
}

\subsection*{\DIFadd{S3 Fig}}
\label{Fig_S3}
{\bf \DIFadd{Venn diagram comparing KAV bins identified by the three different approaches.}} \DIFadd{After applying a variant thinning procedure, 192 100-kb bins (257 variants), 296 bins (300 variants) and 343 bins (442 variants) were identified using arbitrary thresholds for the single-variant, stepwise regression and Bayesian-based multi-variant approaches, respectively. Note in the parentheses listed the unique KAV numbers in the bins.
}

\subsection*{\DIFadd{S4 Fig}}
\label{Fig_S4}
{\bf \DIFadd{Single variant effect and heritability of the 231 KAVs.}} \DIFadd{In panel (a) and (b), histogram distributions of single variant effect and heritability were computed for the 231 KAVs individually. Note that because of linkage disequilibrium (LD), effect and heritability might be repeatedly calculated.
}

\subsection*{\DIFadd{S5 Fig}}
\label{Fig_S5}
{\bf \DIFadd{Characteristics of the ~13M variants and 231 KAVs. }} \DIFadd{In upper panels (a, b and c), pie charts show the physical distance to the nearest gene, origin and type of variation of the $\sim$13M variants used for the GWAS, respectively; in lower panels (d, e and f), show the same characteristics of 231 KAVs. The star indicates significant difference after Chi-square tests.
}

\subsection*{\DIFadd{S6 Fig}}
\label{Fig_S6}
{\bf \DIFadd{Genetic and physical positions of the KAVs across 10 maize chromosomes.}} \DIFadd{Red dots represent the genetic validated KAVs and blue dots are not validated KAVs.
}

\subsection*{\DIFadd{S1 Table}}
\label{Table_S1}
{\bf \DIFadd{KRN trait data of the 6,230 lines.}}

\subsection*{\DIFadd{S2 Table}}
\label{Table_S2}
{\bf \DIFadd{Separate QTL results of IBM and NAM RILs.}} \DIFadd{The markers used in the analysis can be accesed through Panzea website.
}


\subsection*{\DIFadd{S3 Table}}
\label{Table_S3}
{\bf \DIFadd{Four QTL regions exhibit opposite effects in different NAM RIL subpopulations.}}

\subsection*{\DIFadd{S4 Table}}
\label{Table_S4}
{\bf \DIFadd{Joint QTL results of 25 NAM RIL subpopulations.}} \DIFadd{The markers used in the analysis can be accesed through Panzea website.
}

\subsection*{\DIFadd{S5 Table}}
\label{Table_S5}
{\bf \DIFadd{KAVs identified by three GWAS approaches.}} \DIFadd{Column “value” are significant measurements of different approaches, where –log10(P) values for single-variant approach, F-test statistic values for stepwise regression approach, and posterior model frequencies (MF) for Bayesian-based multi-variant approach. Physical positions of the SNPs are based on B73 RefGen v2. Binsize  = 100-kb.
}

\subsection*{\DIFadd{S6 Table}}
\label{Table_S6}
{\bf \DIFadd{Effects of KAVs in the four QTL regions.}} \DIFadd{In QTL regions of order 1 and 2, both positive and negative effects were observed for identified KAVs. 
}

\subsection*{\DIFadd{S7 Table}}
\label{Table_S7}
{\bf \DIFadd{KThe 231 KAVs selected for cross-validation.}} \DIFadd{Note some of the KAVs were identified by multiple approaches. 
}

\subsection*{\DIFadd{S8 Table}}
\label{Table_S8}
{\bf \DIFadd{Favorable allele frequencies and statistical test results of historical lines. }}

\subsection*{\DIFadd{S9 Table}}
\label{Table_S9}
{\bf \DIFadd{Cross-validation samples.}}

\subsection*{\DIFadd{S10 Table}}
\label{Table_S10}
{\bf \DIFadd{Genotypic data for cross-validation population of elite inbred lines.}} \DIFadd{Genotypes were coded using 3 for reference-like, 2 for heterozygotes, 1 for non-reference-like variant types and 0 for missing.
}

\subsection*{\DIFadd{S11 Table}}
\label{Table_S11}
{\bf \DIFadd{Genotypic data for cross-validation population of USDA PI accessions.}}

\subsection*{\DIFadd{S12 Table}}
\label{Table_S12}
{\bf \DIFadd{Genotypic data for cross-validation population of BSLE.}}

\subsection*{\DIFadd{S13 Table}}
\label{Table_S13}
{\bf \DIFadd{Summary of cross-validation results.}} 
\DIFadd{Columns of “Elite1}\_Qvalue\DIFadd{” and “Elite2}\_Qvalue\DIFadd{” are cross-validation results using elite inbred population with and without population structure control. Columns of “USDA}\_Qvalue\DIFadd{” and “BSLE}\_Qvalue\DIFadd{” are cross-validation results using USDA germplasm accessions and BSLE population. Columns “Elite1}\_DOE\DIFadd{”, “Elite2}\_DOE\DIFadd{”, “USDA}\_DOE\DIFadd{” and “BSLE}\_DOE\DIFadd{” are products of direction of effects (DOE) of KAVs in the GWAS population and the cross-validation populations, where positive numbers indicated consistency and negative number indicated inconsistency of DOE. “NA” in the table indicated the data are not available. Significant KAVs were indicated by stars ($\ast$) in the column of “SNPID”.
}

\subsection*{\DIFadd{S14 Table}}
\label{Table_S14}
{\bf \DIFadd{List of candidate genes within KAV-associated chromosomal bins that aligned to evidence supported genes.}}
\DIFaddend 
